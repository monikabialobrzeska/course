...
# Użycie funkcji do obliczania statystyk dla wybranego kraju, z wczesniejszym sprawdzeniem, czy zawodnicy z tego kraju sa w pliku
unikalne_kraje = set()
for skoczek in skoczkowie:
    unikalne_kraje.add(skoczek['kraj'])
# Wybór kraju dla którego obliczane będą statystyki
print('--------------------------------------------------------------------------------------------------------------')
wybrany_kraj=input('Wybierz kraj, dla którego policzone będą statystyki: ').upper()

if wybrany_kraj in unikalne_kraje:
